...
def list_services(project_id, region):
    ...
    try:
                ...
                for service in services_client.list_services(request=list_request):
                    ...
                    if latest_revision_name:
                         try:
                             ...
                             revision_request = run_v2.GetRevisionRequest(name=latest_revision_name)
                             revision_detail = revisions_client.get_revision(request=revision_request)
                             # -----------------------------------------------
                         except exceptions.NotFound:
                             logging.warning(f"Could not find revision {latest_revision_name} for service {service.name.split('/')[-1]}.")
                         except exceptions.PermissionDenied:
                             logging.warning(f"Permission denied to get revision details for service {service.name.split('/')[-1]}.")
        ...
        for service, revision in all_services_data:
            ...
            template_image = "N/A"
            if service.template and service.template.containers and len(service.template.containers) > 0:
            ...
            click.echo(f"    Image:   {template_image}") # Shows image name:tag used for deployment
            # --- NEW: Print the digest ---
            click.echo(f"    Digest:  sha256:{image_digest}") # Shows the exact running image hash
            # ---------------------------
            click.echo("-" * 35)
